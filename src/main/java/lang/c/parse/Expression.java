...
public class Expression extends CParseRule {
	// expression ::= term { expressionAdd | espressionSub }
	...
	public void parse(CParseContext pcx) throws FatalErrorException {
		...
		while (ExpressionAdd.isFirst(tk) || ExpressionSub.isFirst(tk)) {
			if (ExpressionAdd.isFirst(tk)) list = new ExpressionAdd(pcx, term);
			if (ExpressionSub.isFirst(tk)) list = new ExpressionSub(pcx, term);
			list.parse(pcx);
			term = list;
